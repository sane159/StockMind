...
StockMind 是一个面向个人投资者的私有化 AI 股票分析平台。通过接入任意 OpenAI 兼容的大语言模型（DeepSeek、智谱、Ollama 等），结合实时行情、技术指标、资金流向、财经新闻等多维数据，由多个专业 Agent 自动完成盘前研判、盘中监控、盘后复盘全流程，并在关键时刻通过 Telegram、企业微信等渠道推送提醒。
...
支持 Telegram / 企业微信 / 钉钉 / 飞书 / Bark / 自定义 Webhook，可配置多个渠道并按需分配。
...
3. **设置 → 通知渠道**：配置 Telegram Bot 或其他推送渠道（可选）
