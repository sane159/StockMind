...
async def scan_intraday(analyze: bool = False, db: Session = Depends(get_db)):
    ...
    # 只扫描当前开市的A股
    from src.agents.intraday_monitor import is_market_trading
    active_watchlist = watchlist if is_market_trading() else []
    ...
    try:
        ...
        symbols = [s.symbol for s in active_watchlist]
        all_quotes = await collector.get_stock_data(symbols)
    except Exception as e:
        logger.error(f"采集A股行情失败: {e}")
    ...
    async def _load_kline_summary(symbol: str):
        try:
            async with kline_sem:
                return await asyncio.to_thread(
                    lambda: KlineCollector().get_kline_summary(symbol)
                )
        except Exception as e:
    ...
    async def _build_result_item(quote):
        ...
        # 获取技术分析（并发）
        kline_summary = await _load_kline_summary(quote.symbol)
