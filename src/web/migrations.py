...
def _m118_drop_excluded_markets(conn: Connection) -> None:
    """删除 paper_trading_account.excluded_markets 列（仅支持 A股，无需市场过滤）。"""
    try:
        conn.execute(text("ALTER TABLE paper_trading_account DROP COLUMN excluded_markets"))
    except Exception:
        pass  # column already removed or SQLite version too old
...
MIGRATIONS: tuple[Migration, ...] = (
    ...
    Migration(118, "drop_excluded_markets", _m118_drop_excluded_markets),
)
