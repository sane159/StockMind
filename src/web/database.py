...
def _migrate(engine):
    ...
    migrations = [
        ...
        # Phase 3: 持仓增强（已废弃，stocks.invested_amount 已移除）
        # Phase 4: Agent 执行模式
        (
            "agent_configs",
            "execution_mode",
...
INSERT INTO stocks__new (
  id, symbol, name, market, sort_order, created_at, updated_at
)
SELECT
  id, symbol, name, market, COALESCE(sort_order, 0), created_at, updated_at
FROM stocks
"""
                )
            )
