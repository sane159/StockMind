...
class ScreenshotCollector:
    ...
    def _get_url(self, symbol: str, provider: str = "xueqiu") -> str:
        """生成 K 线图页面 URL"""
        if provider == "sina":
            return self._get_sina_url(symbol)
        elif provider == "xueqiu":
            return self._get_xueqiu_url(symbol)
        else:
            return self._get_eastmoney_url(symbol)

    def _get_sina_url(self, symbol: str) -> str:
        """新浪财经 URL"""
        prefix = get_cn_prefix(symbol)
        return f"https://finance.sina.com.cn/realstock/company/{prefix}{symbol}/nc.shtml"

    def _get_eastmoney_url(self, symbol: str) -> str:
        """东方财富 URL"""
        prefix = get_cn_prefix(symbol)
        return f"https://quote.eastmoney.com/{prefix}{symbol}.html"

    def _get_xueqiu_url(self, symbol: str) -> str:
        """雪球 URL"""
    ...
    async def capture(
        ...
        url = self._get_url(symbol, provider)
